...
- Priorizar `int8`/`bigint` para chaves primárias e estrangeiras de negócio.
- Em tabelas novas, usar preferencialmente `id bigint generated by default as identity primary key`.
- Evitar `uuid` como chave primária, salvo justificativa técnica forte documentada neste plano antes da implementação.
- Chaves estrangeiras devem seguir o mesmo tipo da chave primária referenciada; não misturar `uuid` e `bigint` para a mesma entidade.
- Para relacionamento com `auth.users`, usar `uuid` apenas no campo que referencia o usuário autenticado, por exemplo `user_id uuid references auth.users(id)`.
- O campo de autenticação `user_id` não deve substituir o `id bigint` principal de tabelas como `projetse.profiles`.
...
- `id bigint generated by default as identity primary key`
- `organization_id bigint references projetse.organizations(id)`
- `profile_id bigint references projetse.profiles(id)`
...
- `organization_id bigint references projetse.organizations(id)`
- `nome text not null`
- `incorporadora_id bigint references projetse.incorporadoras(id)`
- `cidade text`
- `uf text`
- `endereco text`
- `lote text`
- `quadra text`
- `matricula text`
- `responsavel_profile_id bigint references projetse.profiles(id)`
- `status text`
- `progresso integer`
...
- `id bigint generated by default as identity primary key`
- `empreendimento_id bigint references projetse.empreendimentos(id)`
- `lote_numero text`
- `lote_extenso text`
- `quadra_numero text`
- `quadra_extenso text`
...
- `id bigint generated by default as identity primary key`
- `imovel_id bigint references projetse.imoveis(id)`
- `direcao text`
- `confrontante text`
- `medida text`
- `azimute text`
...
- `uploaded_by_profile_id bigint references projetse.profiles(id)`
- `created_at timestamptz`
...
- `id bigint generated by default as identity primary key`
- `empreendimento_id bigint references projetse.empreendimentos(id)`
- `quadro_tecnico_id bigint references projetse.quadros_tecnicos(id)`
- `bloco text not null`
- `campo text not null`
- `valor text`
...
- `reviewed_by_profile_id bigint references projetse.profiles(id)`
...
- `id bigint generated by default as identity primary key`
- `organization_id bigint references projetse.organizations(id)`
- `modelo_id bigint references projetse.modelos_documento(id)`
...
- `created_by_profile_id bigint references projetse.profiles(id)`
- `created_at timestamptz`

#### `projetse.memorial_secoes`

- `id bigint generated by default as identity primary key`
- `memorial_id bigint references projetse.memoriais(id)`
- `clausula_id bigint references projetse.clausulas(id)`
- `titulo text not null`
- `conteudo text`
- `status text check (status in ('nao_gerada', 'gerada', 'em_revisao', 'com_pendencia', 'aprovada'))`
- `ordem integer`
- `approved_by_profile_id bigint references projetse.profiles(id)`
...
- `created_by_profile_id bigint references projetse.profiles(id)`
- `resolved_by_profile_id bigint references projetse.profiles(id)`
- `created_at timestamptz`
...
- `projetse.profiles.id` é a chave bigint interna do perfil, enquanto `projetse.profiles.user_id` é apenas a referência UUID para `auth.users(id)`.
...
- Tabelas com `organization_id`: acesso permitido se `auth.uid()` corresponder a `projetse.profiles.user_id` de um membro ativo da organização.
